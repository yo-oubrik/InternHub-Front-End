...
const LocationSelector = ({ setPosition }: { setPosition: (position: [number, number]) => void }) => {
  useMapEvents({
    click: (e) => {
...
export const CompanyLocation = () => {
  const [position, setPosition] = useState<[number, number]>([30.406984625615515, -9.53809421587912]);

  return (
    <div className='w-full mt-5 flex flex-col gap-4'>
      <MapContainer className='rounded-lg shadow-sm cursor-pointer' center={position} zoom={20} style={{ height: '700px' , width: '100%'}}>
        <TileLayer
